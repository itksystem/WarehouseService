...
exports.findProductsByCategories = async (categories = [],page=1, limit=10, search = null) => {
  const result = await new Promise((resolve, reject) => {
    db.query(SQL.WAREHOUSE.SQL_GET_PRODUCTS_BY_CATEGORY_ID2, [page, limit, search], (err, result) => {           
      if (err) {
        logger.error(err);
        return reject(err);
      }
...
exports.findProductsByCategoriesByLike = async (categories = [],page=1, limit=10, search = null) => {
  const result = await new Promise((resolve, reject) => {
    db.query(SQL.WAREHOUSE.SQL_GET_PRODUCTS_BY_CATEGORY_LIKE, [page, limit, search], (err, result) => {           
      if (err) {
        logger.error(err);
        return reject(err);
      }
