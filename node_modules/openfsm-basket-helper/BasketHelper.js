...
async function getBasket(userId) {
  ...
  const basketId = basket[0].id;
  const [items] = await db.promise().query(`SELECT wbi.*, w.product_name, w.price, w.description FROM warehouse_basket_item wbi
 LEFT JOIN warehouse w on (w.product_id = wbi.product_id)
WHERE order_id is null and basket_id = ?`, [basketId]);
  return items;
}
