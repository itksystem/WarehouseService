...
async function addItemToBasket(userId, productId, quantity) {
  ...
  const item = await new Promise((resolve, reject) => {
    ...
    return item?.rows[0].quantity + quantity;  
   } catch (err) {
      logger.error(err);
      return quantity;    
  }   
...
async function getProductCountInBasket(userId, productId) {
  try {
      ...
      db.query(
        SQL.BASKET.SQL_UPSERT_BASKET_QUANTITY, [quantity, basketId, productId], (err, result) => {
        if (err) {
           logger.error(err);
           return reject(err);
         }
  ...
  try {
   const item = await new Promise((resolve, reject) => {
    db.query(SQL.BASKET.SQL_FIND_BASKET_ITEM_BY_PRODUCT_ID,  [basketId, productId], (err, result) => {
      ...
      resolve(result?.rows[0] || null); // Предполагается, что поле isConfirmed
    });
  });  
  ...
  await Promise.all(
    items.map(async (item) => {
      try { 
        ...
        item.media_files = media.rows.length > 0 
          ? [new MediaImageDto(media.rows[0])] 
          : [];
      } catch (mediaError) {
...
async function getBasketId(userId) {    
  const basket = await new Promise((resolve, reject) => {
    db.query(SQL.BASKET.SQL_FIND_BASKET_BY_USER_ID, [userId], (err, result) => {
      ...
      resolve(result?.rows || null); // Предполагается, что поле isConfirmed
    });
  });  
  let basketId = (basket?.length > 0) ? basket[0].id : await createBasket(userId);     
  return basketId;
}
...
async function getBasketOrder(userId, orderId) {
  const basketId = await getBasketId(userId);
  const [items] = await db.promise().query(SQL.BASKET.SQL_GET_BASKET_ORDER_ITEMS, [orderId, basketId]);  
...
async function productBasketRemove(userId, productId) {
  try {   
   ...
   if(!result) throw new WarehouseError(500, MESSAGES[LANGUAGE].ERROR_REMOVING_PRODUCT_FROM_BASKET ); 
     ...
     logger.error(`${MESSAGES[LANGUAGE].ERROR_REMOVING_PRODUCT_FROM_BASKET} productId ${productId} basketId ${basketId} => ${err}`);
     return false; 
   };
  return true; 
}
