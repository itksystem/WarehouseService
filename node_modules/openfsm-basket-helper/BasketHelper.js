    ...
    await Promise.all(
      items.map(async (item) => {
        try {
          ...
          item.media_files = media ? [new MediaImageDto(media)] : [];
        } catch (mediaError) {
...
async function getBasketOrder(orderId = null, basketId = null) {
  try {    
    // Выполняем запрос на получение товаров из корзины userId, orderId     
    if(!orderId || !basketId) throw(`warehouse.getBasketOrder orderId ${userId} basketId ${orderId}`);
    const { rows: items } = await db.query(SQL.BASKET.SQL_GET_BASKET_ORDER_ITEMS, [orderId, basketId]);
   
    // Обрабатываем каждый товар и добавляем медиа файлы
    ...
    logger.error(err);
    throw new Error('Ошибка при получении состава заказа');
  }
}
...
module.exports = { addItemToBasket, removeItemFromBasket, getBasket, orderCreate, getBasketOrder, 
  getBasketId, getProductCountInBasket, getOrderDetails, productBasketRemove, getWarehouseProductCount, orderDecline};