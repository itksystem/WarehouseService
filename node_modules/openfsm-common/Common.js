...
const SQL_GET_PRODUCT_BY_PRODUCT_ID = 
        "SELECT product_id, owner_id, product_name, price, description, quantity, reserved_quantity, created_at, updated_at " +
        "FROM warehouse " +
        "WHERE product_id = ?";
...
const SQL_UPDATE_BASKET_REMOVE_ITEM_BY_PRODUCT_ID = `UPDATE warehouse_basket_item SET quantity = 0 WHERE order_id is null and basket_id = ? AND product_id = ?`;
...
const SQL_GET_BASKET_ORDER_ITEMS = `SELECT 
    wbi.product_id,
    wbi.quantity,        
    w.product_name, w.price, w.description FROM warehouse_basket_item wbi
    LEFT JOIN warehouse w on (w.product_id = wbi.product_id)
    WHERE wbi.quantity > 0 and order_id = ? and basket_id = ?`;
...
SQL_GET_BASKET_ORDER_ITEMS,
// 22-12-2024
SQL_UPDATE_BASKET_REMOVE_ITEM_BY_PRODUCT_ID,
